...
const Chat = (props: Props) => {
  ...
  useEffect(() => {
    const processChatThread = async () => {
      if (chatThread && chatThread.chats.length > 0) {
        ...
        if (lastChatId !== lastProcessedChatRef.current && !lastChat.answer) {
          // Skip all API calls and just set mode to chat
          if (!lastChat.mode) {
            dispatch(
              updateMode({
                threadId: id,
            ...
            // Directly call handleAnswer without any API processing
            await handleAnswer(lastChat);
  ...
  if (isFetching) {
    ...
    setTimeout(() => {
      if (isFetching) {
        // If still fetching after timeout, show 404
        return <Chat404 />;
      }
    }, 5000);
