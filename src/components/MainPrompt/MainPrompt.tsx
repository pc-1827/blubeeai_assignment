...
const MainPrompt = () => {
  ...
  const handleSend = () => {
    if (text.trim() !== "") {
      ...
      // Create chat object with the answer already set
      const chatObject: Chat = {
        mode: "chat", // Default to chat mode in test mode
        question: text.trim(),
        answer: TEST_MODE_MESSAGE, // Set the answer directly
        query: focus.query,
        ...(fileInfo && { fileInfo }),
      };

      // Skip Firebase in test mode, create chat with both messages
      dispatch(createChatThread({
        id,
        chat: chatObject,
        messages: [
          { role: "user", content: text.trim() },
          { role: "assistant", content: TEST_MODE_MESSAGE }
        ],
        createdAt: new Date() // Add timestamp for history sorting
      }));
  ...
  const handleFile = async () => {
    ...
    fileInput.addEventListener("change", async () => {
      if (fileInput.files && fileInput.files.length > 0) {
        toast.error("This feature is currently disabled in test mode.", {
          position: "top-center",
          style: {
            padding: "6px 18px",
            color: "#fff",
