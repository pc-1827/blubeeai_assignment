...
const chatSlice = createSlice({
  ...
  reducers: {
    createChatThread: (
      state,
      action: PayloadAction<{ id: string; chat: Chat; messages?: Message[]; createdAt?: Date }>
    ) => {
      const { id, chat, messages = [], createdAt } = action.payload;
      state.threads[id] = {
        id,
        chats: [chat],
        messages: messages,
    ...
    addChatThread: (
      ...
      action: PayloadAction<{
        ...
        messages: Message[];
        shared?: boolean;
        createdAt?: Date; // Make createdAt optional for backward compatibility
      }>
    ) => {
      const { id, chats, messages, shared = false, createdAt } = action.payload;
      state.threads[id] = {
        id,
        chats,
        messages,
        shared,
        createdAt: createdAt || new Date() // Use provided value or default to current date
      };
    },
